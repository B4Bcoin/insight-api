...
UtilsController.prototype.estimateSmartFee = function(req, res) {
  ...
  async.map(nbBlocks, function(n, next) {
    var num = parseInt(n);
    // Insight and b4bcoin JSON-RPC return b4bcoin for this value (instead of satoshis).
    self.node.services.b4bd.estimateSmartFee(num, conservative, function(err, fee) {
      if (err) {
        return next(err);
      }
      next(null, [num, fee]);
