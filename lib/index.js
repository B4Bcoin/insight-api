...
/**
 * A service for b4bcore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinShort = options.coinShort || 'B4B';
...
InsightAPI.dependencies = ['b4bd', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.b4bd.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.b4bd.on('block', this.blockEventHandler.bind(this));
...
InsightAPI.prototype.getPublishEvents = function() {
  return [
    ...
    },
    {
        ...
        scope: this,
        subscribe: function (emitter) {
            this.subscribe(emitter, 'b4b');
        }.bind(this),
        unsubscribe: function (emitter) {
            this.unsubscribe(emitter, 'b4b');
        },
        extraEvents: ['b4b/tx', 'b4b/block']
    }
  ];
};
...
InsightAPI.prototype.blockEventHandler = function(hashBuffer) {
  ...
  if (self.subscriptions.b4b.length) {
      ...
      this.blocksController.getBlockByHash(hashBuffer.toString('hex'), function (err, block) {
          ...
          async.mapSeries(block.tx, function(txid, next) {
              ...
              // Notify b4b subscribers
              for (var i = 0; i < self.subscriptions.b4b.length; i++) {
                  self.subscriptions.b4b[i].emit('b4b/block', {
                      block: block,
...
InsightAPI.prototype.transactionEventHandler = function(txBuffer) {
   ...
   if (this.subscriptions.inv.length || this.subscriptions.b4b.length) {
       ...
       if (this.subscriptions.b4b.length) {
           var transformedTrx = this.txController.transformB4bTransaction(tx);

           for (var i = 0; i < this.subscriptions.b4b.length; i++) {
               this.subscriptions.b4b[i].emit('b4b/tx', transformedTrx);
           }
